...
public class PlayerProfile {
    ...
    public void sendStats(@Nonnull CommandSender sender) {
        ...
        sender.sendMessage("");
        sender.sendMessage(ChatColors.color("&7玩家統計資料：&b" + name));
        sender.sendMessage("");
        sender.sendMessage(ChatColors.color("&7稱號：" + ChatColor.AQUA + getTitle()));
        sender.sendMessage(ChatColors.color("&7研究進度：" + NumberUtils.getColorFromPercentage(progress) + progress + "&r%" + ChatColor.YELLOW + '（' + unlockedResearches + " / " + allResearches + '）'));
