...
public class CargoInputNode extends AbstractFilterNode {
    ...
    protected void updateBlockMenu(BlockMenu menu, Block b) {
        ...
        if (!BlockStorage.hasBlockInfo(b) || roundRobinMode == null || roundRobinMode.equals(String.valueOf(false))) {
            menu.replaceExistingItem(24, new CustomItem(SlimefunUtils.getCustomHead(HeadTexture.ENERGY_REGULATOR.getTexture()), "&7循環模式: &4\u2718", "", "&e> 點擊以啟用循環模式", "&e(物品將在頻道上平均分配)"));
            ...
            menu.replaceExistingItem(24, new CustomItem(SlimefunUtils.getCustomHead(HeadTexture.ENERGY_REGULATOR.getTexture()), "&7循環模式: &2\u2714", "", "&e> 點擊以禁用循環模式", "&e(物品將在頻道上平均分配)"));
            menu.addMenuClickHandler(24, (p, slot, item, action) -> {
