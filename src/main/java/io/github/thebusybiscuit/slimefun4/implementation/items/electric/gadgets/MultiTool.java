    ...
    protected ItemUseHandler getItemUseHandler() {
        return e -> {
            ...
            if (!p.isSneaking()) {
                ...
                SlimefunPlugin.getLocalization().sendMessage(p, "messages.multi-tool.mode-change", true, msg -> msg.replace("%device%", "多功能工具").replace("%mode%", ChatColor.stripColor(itemName)));
